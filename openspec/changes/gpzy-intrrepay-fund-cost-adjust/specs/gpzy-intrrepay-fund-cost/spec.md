...
### Requirement: 还息计划扣收交割流水写入合约外部凭证号
系统 SHALL 在扣收完成生成 `node_settdetail` 时，对 `busitype` 为 `015032` 或 `025032` 且 `pathid='GPZYDEBT_INTRREPAYPLAN'` 的还息计划来源扣收记录写入合约外部凭证号，并 MUST 将 `node_settdetail.outvoucherno` 设置为对应 `node_debtdetail.matchcode`，即股票质押合约号 `gpzysno`。

#### Scenario: 一次扣收写入合约号
- **WHEN** 系统为 `busitype` 为 `015032` 或 `025032` 且 `pathid='GPZYDEBT_INTRREPAYPLAN'` 的一次扣收记录生成交割流水
- **THEN** 系统 MUST 生成对应 `node_settdetail`
- **AND** 系统 MUST 将 `node_settdetail.outvoucherno` 写为该计划负债 `node_debtdetail.matchcode`

#### Scenario: 二次扣收写入合约号
- **WHEN** 系统为 `busitype` 为 `015032` 或 `025032` 且 `pathid='GPZYDEBT_INTRREPAYPLAN'` 的二次扣收记录生成交割流水
- **THEN** 系统 MUST 生成对应 `node_settdetail`
- **AND** 系统 MUST 将 `node_settdetail.outvoucherno` 写为该计划负债 `node_debtdetail.matchcode`

#### Scenario: 非计划来源不启用专用赋值
- **WHEN** 系统为非 `GPZYDEBT_INTRREPAYPLAN` 来源的 `015032` 或 `025032` 负债生成交割流水
- **THEN** 系统 MUST 不执行还息计划来源负债专用的 `outvoucherno` 赋值分支
- **AND** 系统 MUST 保持既有交割流水字段赋值行为
