...
系统 SHALL 在计划来源负债 paidamt 大于 0 时，按 gpzy-intrrepay-contract-deal 既有利息明细抵扣规则更新 node_gpzydebt_intrdetail，并 MUST 在产生完成利息明细时刷新 node_gpzydebt 的 lastallrepayintrdate 和 allrepayintrdatedayleft。系统 MUST 仅当最后完成日下一条利息明细确实发生偿还时，将 allrepayintrdatedayleft 写为该明细剩余未还利息；下一条未偿还或不存在下一条时写 0。系统 MUST 在 paidamt 等于 0 时不抵扣利息明细且不推进主合约利息完成信息。
...
- **AND** 当最后完成日期后一条利息明细确实发生偿还时，系统 MUST 将 node_gpzydebt.allrepayintrdatedayleft 更新为该明细的剩余未还利息
- **AND** 当最后完成日期后一条利息明细未发生偿还或不存在下一条明细时，系统 MUST 将 node_gpzydebt.allrepayintrdatedayleft 更新为 0
...
- **AND** 系统 MUST 仅当 allrepayintrdatedayleft 大于 0 时恢复完成日下一日明细的高精度已还金额
