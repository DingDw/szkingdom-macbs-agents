...
- 复用 gpzy-intrrepay-contract-deal 的利息明细抵扣、高精度尾差和主合约利息完成信息刷新规则；`allrepayintrdatedayleft` 仅在最后完成日下一条明细确实发生偿还时记录剩余利息，否则写 0。
...
利息明细抵扣、高精度尾差和主合约利息完成信息刷新规则应复用或抽成公共 helper，确保扣收后处理与 gpzy-intrrepay-contract-deal 行为一致；特别是 `allrepayintrdatedayleft` 只记录最后完成日下一条已发生偿还明细的剩余利息，下一条未偿还或不存在下一条时写 0。
...
利息明细恢复规则：使用 busidate=当前 busidate 且 debtdealpoint=1 的对应合约中的 lastallrepayintrdate 和 allrepayintrdatedayleft，按 gpzy-intrrepay-contract-deal 的恢复赋值逻辑回退明细；仅当 allrepayintrdatedayleft 大于 0 时恢复完成日下一日明细的高精度已还金额，否则完成日及以后明细均恢复为未完成未偿还。
