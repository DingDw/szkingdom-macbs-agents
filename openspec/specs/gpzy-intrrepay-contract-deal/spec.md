...
定义国信股票质押违约处置自动偿还利息的日终合约处理规则，包括国信特色处理启用、还息计划和利息明细缓存、自动偿还分配、冻结解冻流水、计划负债生成以及恢复重做，确保仅在国信环境启用扩展逻辑，其他客户保持既有股票质押合约处理行为。
...
系统 SHALL 在完成 `node_gpzydebt_intrrepayplan` 和 `node_gpzydebt_intrdetail` 更新后，回写 T 日 `node_gpzydebt` 的最后完成利息偿还日和下一日剩余利息。
...
- **AND** MUST 将 `node_gpzydebt.allrepayintrdatedayleft` 更新为最后完成日下一条利息明细的 `todayintr - repayintr`
...
- **AND** 对完成日期下一日的利息明细，系统 MUST 将 `repayintr` 恢复为 `todayintr - T-1 allrepayintrdatedayleft`
